...
integer i=1, k=0, data_loaded = 0, m=0;

fft #(.FFT_SIZE(`N), .FFT_SIZE_LOG(`r), .WIDTH(`W), .DECIMAL(`D)) fft (clk, reset, start,data_en, ready, inbuffer_R, inbuffer_I, outbuffer_R, outbuffer_I);
...
begin
    #50 data_en = ~data_en;
    if(start == 1'b1 && ready == 1'b0 && data_loaded ==1'b0)begin      
        if(i < `N && data_en == 1'b1)begin
            inbuffer_R = data_R[i];
            inbuffer_I = data_I[i];
            i = i+1;
        end
