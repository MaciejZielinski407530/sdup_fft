...
module stage0 #(parameter FFT_SIZE = 8, parameter FFT_SIZE_LOG = 3, parameter WIDTH = 18, parameter DECIMAL = 10)(
input clk,
...
butterfly #(.WIDTH(WIDTH), .DECIMAL(DECIMAL), .MAX_FFT_SIZE(FFT_SIZE), .MAX_FFT_SIZE_LOG(FFT_SIZE_LOG))
    butterfly_0(clk, start[0], ready[0], R0_in, I0_in, R4_in, I4_in, twiddle_R[0], twiddle_I[0],R0_out, I0_out, R4_out, I4_out);

butterfly #(.WIDTH(WIDTH), .DECIMAL(DECIMAL), .MAX_FFT_SIZE(FFT_SIZE), .MAX_FFT_SIZE_LOG(FFT_SIZE_LOG))
    butterfly_1(clk, start[1], ready[1], R1_in, I1_in, R5_in, I5_in, twiddle_R[0], twiddle_I[0],R1_out, I1_out, R5_out, I5_out);
    
butterfly #(.WIDTH(WIDTH), .DECIMAL(DECIMAL), .MAX_FFT_SIZE(FFT_SIZE), .MAX_FFT_SIZE_LOG(FFT_SIZE_LOG))
    butterfly_2(clk, start[2], ready[2], R2_in, I2_in, R6_in, I6_in, twiddle_R[0], twiddle_I[0],R2_out, I2_out, R6_out, I6_out);
    
butterfly #(.WIDTH(WIDTH), .DECIMAL(DECIMAL), .MAX_FFT_SIZE(FFT_SIZE), .MAX_FFT_SIZE_LOG(FFT_SIZE_LOG))
    butterfly_3(clk, start[3], ready[3], R3_in, I3_in, R7_in, I7_in, twiddle_R[0], twiddle_I[0],R3_out, I3_out, R7_out, I7_out);
